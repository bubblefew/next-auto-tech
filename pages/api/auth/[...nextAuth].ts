...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  const action = req.query["nextAuth"]?.[0]; // Optional chaining used here

  if (action && req.method === HTTP_METHOD_POST && action === "signin") {
    return signin(req, res);
  } else if (action && req.method === HTTP_METHOD_GET && action === "signout") {
    return signout(req, res);
  } else if (action && req.method === HTTP_METHOD_GET && action === "session") {
